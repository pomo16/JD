$(function(){
	...
	$("#add_product_picture").click(function(){
		...
		if($("#picture_input").val() != ""){
			url += "?pictureName=" + $("#picture_input").val();
		}
	...
	$("#add_product_picture_update").click(function(){
		$('#file_upload_message_update').html("");
		$('#progress_update').css("display","none");
		var url = "FileUploadServlet";
		if($("#picture_input_update").val() != ""){
			url += "?pictureName=" + $("#picture_input_update").val();
		}
			...
			{field:'picture',title:'图片',width:100,align:'center',
				formatter: function(value,row,index){
					if(value==""){
						...
						return "<img style='width:80px;border-radius:5px;border:1px solid #eee' src='../common/image/product/" + value + "?" + new Date().getTime() + "'>";
					}
				}	
			},
			{field:'id',title:'操作',width:100,
				formatter: function(value,row,index){
					// value是数据库中的id;row是行对象,phone和userName这些都是它的属性;index是序号,从零开始计数
					return "<a onclick='creatUpdateProduct(" + index + ")' class='jd_btn'>修改</a>";
				}
			}
		]],
...
function validateForm(){
	$("#product_form").validate({
		submitHandler: function () {
			$.post("AddProductServlet", $("#product_form").serialize(), function(data){
				if(data=="ok"){
					$("#product_window").window('close');
		...
		rules:{
			productCode:{
				required:true,
				rangelength:"[4,12]",
				remote:"CheckProductCodeServlet"
			},
	...
	$("#update_product_form").validate({
		submitHandler: function () {
			$.post("UpdateProductServlet", $("#update_product_form").serialize(), function(data){
				if(data=="ok"){
					$("#update_product_window").window('close');
					$.messager.alert("修改成功","【" + $("#productNameUpdate").val() + "】商品添加成功！","info");
					$("#product_table").datagrid("reload");
				}
			});
		},
...
function creatUpdateProduct(index){
	$('#file_upload_message_update').html("");
	$('#progress_update').css("display","none");
	...
	$("#picture_img_update").attr("src","../common/image/product/" + row.picture + "?" + new Date().getTime());
}
	...
	$("#fileupload").fileupload({
        ...
        messages: {
	    	...
	    	if(data.result.key == "ok"){
	    		$("#file_upload_message" + update).append("<span style='color:green;margin-left:50px'>上传完毕</span>");
	    		$("#picture_img" + update).css("display","inline");
	    		$("#picture_img" + update).attr("src","../common/image/product/" + data.result.fileName + "?" + new Date().getTime());
	    ...
	    processfail:function (e, data) {
	    	...
	    	$("#file_upload_message" + update).append("<span style='color:red'>" + fileName + " " + error + "</span>");
	    },
	    progress: function (e, data) {
	    	...
	    	$("#file_upload_message" + update).html(fileName + " 已上传：" + loaded + " 大小：" + total + " 速度：" + bitrate);
	    }
	});
}
