...
public class MD5Encrypt {
	public static byte[] encryptByMD5(String password) {
		...
		System.out.println("最终数据库中保存密码=" + Arrays.toString(resultPassword));
	...
	public static boolean validatePassword(String password, byte[] resultPassword) {
		...
		System.out.println("密码校验中的随机值=" + Arrays.toString(randomBytes));
		MessageDigest md = null;
		try {
			md = MessageDigest.getInstance("MD5");			
		} catch (NoSuchAlgorithmException e) {
		...
		//获得加密之后的密码
		byte[] passwordBytes = md.digest();
		System.out.println("密码校验中加密之后的密码=" + Arrays.toString(passwordBytes));
