		    ...
		    {field:'picture',title:'图片',width:100,align:'center',
		    	formatter: function(value,row,index){
		    		if(value==""){
						...
						return "<img src='../common/images/product/" + value + "?" + new Date().getTime() + "' style='width:80px;border-radius:5px;border:1px solid #ddd'/>";
		    		}
				}
		    },
...
function validateForm(){
	$("#product_form").validate({
		submitHandler: function () {			
			$.post("product/addProduct.do",$("#product_form").serialize(),function(data){
				if(data=="ok"){					
					$("#product_window").window('close');
	...
	$("#update_product_form").validate({
		submitHandler: function () {
			$.post("product/updateById.do",$("#update_product_form").serialize(),function(data){
				if(data=="ok"){					
					$("#update_product_window").window('close');
