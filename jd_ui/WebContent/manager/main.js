$(function() {
	...
	$("#exit_btn").click(function() {
		// $.message.alert('退出','您确定要退出系统！','info');
		$.messager.confirm('退出','您确定要退出系统？',function(r){
			if (r){
				window.location = "../maintain/ExitServlet"
			}
		});
	});
});
...
function addTab(title, icon, url) {
	if ($('#jd_tabs').tabs('exists', title)) {
		...
		//刷新tabs
		var tab = $('#jd_tabs').tabs('getSelected'); // get selected panel
		$('#jd_tabs').tabs('update', {
			tab : tab,
			options : {
				//href: url  // the new content URL,is a bug
				content : tabCotent(url)
			}
		});
	} else {
		$('#jd_tabs').tabs('add', {
